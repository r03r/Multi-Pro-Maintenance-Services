...
class Element {
  ...
  querySelectorAll(s){if(s==='a, button, input'||s==='a, button')return this.focusables||[];return []}
...
x.tick(800);assert.equal(x.eval('chatStep'),'service');let before=x.el('chat-messages').children.length;x.ctx.chatPickService('paint');x.ctx.chatPickService('paint');assert.equal(x.el('chat-messages').children.filter(b=>b.className.includes('bubble-user')).length,1);x.tick(800);
...
x.ctx.openChat();x.ctx.chatRestart();x.ctx.chatRestart();x.tick(800);assert.equal(x.el('chat-messages').children.length,2);x.ctx.chatPickService('paint');x.tick(800);x.ctx.chatPickCity('LA');x.tick(800);x.ctx.chatPickBudget(chip('Budget'));x.tick(800);x.ctx.chatPickTimeline(chip('Now'));x.ctx.closeChat();const restored=x.doc.activeElement;x.tick(800);assert.equal(x.doc.activeElement,restored);
